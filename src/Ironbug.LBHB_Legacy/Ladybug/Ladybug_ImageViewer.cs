...
using Ironbug.LBHB_Legacy.Properties;
    ...
    public class Ladybug_ImageViewer : GH_Component
    {
        ...
        private string CheckImg(string filePath)
        {
            ...
            if (isHDR)
            {
                ...
                if (isNewHDR)
                {
                    var radTiff = new RaTiff_Legacy(filePath, tiffFile);
                    radTiff.Execute();
                    if (!File.Exists(tiffFile))
                    {
        ...
        {
            get { return new Guid("53FB0C86-1AFB-4B28-AD2B-3916CF2997BB"); }
        }
        ...
        private IEnumerable<double> GetPValues(string HDRImg)
        {
            ...
            var pvalue = new PValue_Legacy(HDRImg);
